...
// Deleting a single user, using id as paramter
...
// Find User by email
func (repositories Users) FindByEmail(userEmail string) (models.User, error) {
	row, err := repositories.db.Query("select id, password from users where email = ?", userEmail)
